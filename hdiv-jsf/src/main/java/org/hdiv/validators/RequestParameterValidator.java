...
/**
 * Validates that all parameters received in a request are valid and that
 * no extra parameters has been added
 * 
...
public class RequestParameterValidator implements ComponentValidator {
	...
	/**
	 * Verifies that all the received parameters correspond to an attribute
	 * of the form that has been sent 
	 * 
	 * @param formComponent
	 * @return
	 */
	private ValidationError validateRequestParameters(UIForm formComponent) {
		...
		List clientIds = getClientIds(formComponent);
		...
		Map requestParameters = fContext.getExternalContext().getRequestParameterMap();
		Iterator it = requestParameters.entrySet().iterator();
		while (it.hasNext()) {
			String requestParamName = ((Entry) it.next()).getKey().toString().trim();
	 ...
	 * @param component
	 */
	private List getClientIds(UIForm component) {
	...
	/**
	 * A�ade en 'clientIds' todas las ids de los componentes susceptibles de
	 * crear par�metros en la petici�n.
	 * 
	 * Adds to 'clientIds' all the component ids susceptible to creating
	 * parameters in the request
	 * 
	 * @param component
	 */
	private void getAllClientIds(UIComponent component, List clientIds) {
