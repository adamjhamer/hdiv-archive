...
public abstract class AbstractValidatorHelper implements IValidationHelper {
	...
	private IState restoreState(HttpServletRequest request, String target) {
		...
		if (requestState == null) {
			this.logger.log(HDIVErrorCodes.HDIV_PARAMETER_NOT_EXISTS, target, this.hdivParameter, null);
			return null;
		}
		...
		try {
			if (stateUtil.isMemoryStrategy(requestState)) {
				...
				if (!this.validateHDIVSuffix(requestState)) {
					this.logger.log(HDIVErrorCodes.HDIV_PARAMETER_INCORRECT_VALUE, target, this.hdivParameter,
							requestState);
					return null;
				}
			}
			...
			this.logger.log(e.getMessage(), target, this.hdivParameter, requestState);
			return null;
		}
	}
...
//		Pattern p = Pattern.compile("[0-9]+");
	...
	public void setHdivParameter(String hdivParameter) {
		this.hdivParameter = hdivParameter;
	}
