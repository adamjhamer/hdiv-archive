...
public abstract class AbstractMultipartConfig implements IMultipartConfig {
	...
	/**
	 * Converts a size value from a string representation to its numeric value. The
	 * string must be of the form nnnm, where nnn is an arbitrary decimal value, and
	 * m is a multiplier. The multiplier must be one of 'K', 'M' and 'G',
	 * representing kilobytes, megabytes and gigabytes respectively. If the size
	 * value cannot be converted, for example due to invalid syntax, the supplied
	...
	/**
	 * Returns the maximum allowable size, in bytes, of an uploaded file. The value
	 * is obtained from the Spring configuration.
	 * 
